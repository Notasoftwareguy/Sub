...
class SubCommunicator:
    ...
    def __init__(self, host: str = 'localhost', port: int = 5000, encoding: str = 'utf-8', logger=None):
        """
    ...
    def start_server(self, backlog: int = 1) -> None:
        ...
        if self.logger: self.logger.info(f"Server listening on {self.host}:{self.port}")
    ...
    def _accept_connection(self) -> None:
        ...
        try:
            while self.is_listening:
                ...
                if self.logger: self.logger.info(f"Connected from {addr}")
            ...
            if self.logger: self.logger.info(f"Error accepting connections: {e}")
    ...
    def _handle_client(self, conn: socket.socket, addr) -> None:
        ...
        try:
            ...
            if self.connection is conn:
                self.connection = None
            if self.logger: self.logger.info(f"Disconnected from {addr}")
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if self.logger: self.logger.info(f"Connected to {self.host}:{self.port}")
    ...
    def _receive_loop(self, conn: Optional[socket.socket] = None) -> None:
        ...
        while True:
            try:
                ...
                if self.logger: self.logger.info(f"Error receiving: {e}")
                break
    ...
    def send(self, message: str) -> bool:
        ...
        if not self.connection:
            if self.logger: self.logger.info("No active connection")
            return False
        ...
        try:
            ...
            if self.logger: self.logger.info(f"Error sending: {e}")
            return False
